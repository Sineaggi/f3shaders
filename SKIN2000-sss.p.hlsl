...
float4 main(
  ...
  float4 worldPos : TEXCOORD5, // World position
  float4 worldNormal : TEXCOORD7 // World normal
  ) : COLOR0
{
  // Texture loading
  ...
  return float4(deltaWorldPosition, deltaWorldPosition, deltaWorldPosition, 1);
  //float OneOverR = n / p;
  //float NdotL = 0;
  float c = (deltaWorldNormal / deltaWorldPosition) * 0.04;
  //float4 sss = tex2D(SSS, float2(NdotL, OneOverR));
